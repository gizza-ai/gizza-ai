...
// Inputs are the shared declarative controls: #in-datetime is a native
// datetime-local picker, #in-from a timezone datalist, and the target zones a
// generic tag-list (.tool-tags pills + .tool-tags-search, backed by the hidden
// #in-to that carries the comma-joined value). Reset is the generic #tool-reset.
test('timezone-convert converts New York to Tokyo', async ({ page }) => {
  ...
  await page.locator('.tool-tag-del').first().click();

  // Add Asia/Tokyo target tag (typing a full vocab value auto-adds it)
  await page.fill('.tool-tags-search', 'Asia/Tokyo');
...
test('timezone-convert handles a half-hour zone (India)', async ({ page }) => {
  ...
  // Add Asia/Kolkata
  await page.fill('.tool-tags-search', 'Asia/Kolkata');
...
test('timezone-convert query-param deep-link prefills + computes', async ({ page }) => {
  ...
  // The datetime-local input normalizes the space separator to T
  await expect(page.locator('#in-datetime')).toHaveValue('2024-01-10T14:30', {
    timeout: 15000,
  });
  // Deep-linked target renders as a pill
  await expect(page.locator('.tool-tag-pill')).toContainText('Asia/Tokyo');
...
test('timezone-convert supports multi-target planner', async ({ page }) => {
  ...
  await page.fill('.tool-tags-search', 'Asia/Tokyo');
  await page.fill('.tool-tags-search', 'Europe/London');
  await expect(page.locator('#in-to')).toHaveValue('Asia/Tokyo, Europe/London');
...
test('timezone-convert page has smart defaults on load', async ({ page }) => {
  ...
  await expect(page.locator('#in-to')).toHaveValue('UTC');
  ...
  // The legacy custom reset is replaced by the generic #tool-reset chrome
