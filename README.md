...
`SKILL.md` (repo root, generated by `gizza gen-skill`) documents the
`gizza tool …` contract plus every tool's schema, so an LLM agent can drive the
tools without reading source. Regenerate it after adding or changing a tool — CI
fails on drift via `gizza gen-skill --check`.
