//! Browser-facing wasm-bindgen wrapper for /tools/round-decimals/.
...
pub fn run(
    ...
    let dec: i64 = if decimals.trim().is_empty() {
        2
    } else {
        decimals
            .trim()
            .parse()
            .map_err(|_| JsValue::from_str("decimals must be a whole number between 0 and 10"))?
    };
    let mode = if mode.is_empty() { "half_up" } else { mode };
    let delim = if delimiter.is_empty() { "," } else { delimiter };
    round_csv(data, dec, mode, columns, truthy(header), delim, truthy(trailing_zeros))
