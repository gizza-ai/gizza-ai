...
// (media, min_bitrate, max_bitrate, units, target).
let selectedBase64 = "";
let runSeq = 0;
...
export function setup(ctx) {
  ...
  function run() {
    const seq = ++runSeq;
    // Nothing to measure until a file is chosen — stay quiet rather than
    // erroring while the user is still filling in the range.
    if (!selectedBase64) {
