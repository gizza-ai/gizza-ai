//! Browser-facing wasm-bindgen wrapper for /tools/salsa20-cipher/.
//! Field order MUST match meta.toml: data, operation, key, nonce, key_format, counter, format.
...
pub fn run(
    ...
    let r = match operation.trim().to_ascii_lowercase().as_str() {
        "decrypt" => decrypt(data, key, nonce, kf, ctr, fmt),
        _ => encrypt(data, key, nonce, kf, ctr, fmt),
    };
