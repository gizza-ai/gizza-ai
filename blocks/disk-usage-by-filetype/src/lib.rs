//! gizza-ai/disk-usage-by-filetype — chat skill block. Aggregates a pasted file
//! listing's bytes by extension (or file-type category) and renders a sorted bar
//! chart, table, CSV, JSON or colored SVG. Chat schema single-sourced from
    ...
    ToolDescriptor::new(Input::None)
        ...
        .param(
            Param::integer("top_n")
                ...
                .min(1.0)
                .max(200.0)
                .describe("How many rows to show before the rest are folded into a single `(other N)` row that keeps their bytes in the total. Default 15. Set it to 5 for a headline summary, or to 200 to list every type separately."),
        )
        .param(
            Param::enumv("units", ["binary", "si", "bytes"])
                .default("binary")
                .describe("How sizes are printed. \"binary\" (default) is the 1024-based KiB/MiB/GiB that `du` and file managers show. \"si\" is the 1000-based kB/MB/GB that drive makers use. \"bytes\" prints the raw byte count with no unit, for spreadsheets. Percentages and the JSON/CSV `bytes` column are unaffected."),
        )
        .param(
            Param::integer("chart_width")
                .default(32)
                .min(8.0)
                .max(120.0)
                .describe("Width in characters of the longest bar in the `chart` output, from 8 to 120 (default 32). The biggest type fills the whole bar and everything else is drawn to scale, in eighth-of-a-character steps. Ignored by the table, csv, json and svg formats."),
        )
        .param(
            Param::boolean("skip_folders")
                .default(true)
                .describe("Ignore lines that describe a folder rather than a file. On by default: `du -a` prints a line per folder whose size already includes everything under it, so counting those would double- or triple-count the same bytes. A line counts as a folder when the path ends in a slash, when `ls -l` marked it `d`, or when another listed path sits inside it. Turn it off when your listing has no folder rows and you want every line counted verbatim."),
        )
        .param(
            Param::boolean("ignore_case")
                .default(true)
                .describe("Treat `.JPG`, `.Jpg` and `.jpg` as one extension and print it lowercase. On by default, because camera and Windows exports mix cases for the same file type. Turn it off to keep the exact spelling of each extension as separate rows."),
        )
        .param(
            Param::enumv("format", ["chart", "table", "csv", "json", "svg"])
                .default("chart")
                .describe("Output shape. \"chart\" (default) is the monospace bar chart: type, size, share, bar and file count per row. \"table\" is the same numbers as aligned columns with a TOTAL row. \"csv\" gives `name,bytes,size,percent,files` for a spreadsheet. \"json\" returns {group_by, units, total_bytes, total_size, total_files, skipped_folders, ignored_lines, groups[]}. \"svg\" returns a colored horizontal bar chart as SVG source you can paste into a doc, ticket or README."),
        )
}
...
#[wafer_block(
    ...
    summary = "Add up a pasted file listing by extension or file-type category and chart the biggest space users",
    skill(
...
impl DiskUsageByFiletype {
    fn handle(_msg: Message, body: Vec<u8>) -> GuestResult {
        match run_skill(&body, "disk-usage-by-filetype", |a: Args| {
            let opts = Options {
                group_by: a.group_by,
                sort_by: a.sort_by,
                order: a.order,
                top_n: a.top_n.round().clamp(0.0, 1000.0) as u32,
                units: a.units,
                chart_width: a.chart_width.round().clamp(0.0, 1000.0) as u32,
                skip_folders: a.skip_folders,
                ignore_case: a.ignore_case,
                format: a.format,
            };
            gizza_ai_disk_usage_by_filetype_core::run(&a.listing, &opts)
