//! gizza-ai/gost-magma-cipher — encrypt or decrypt text with the GOST 28147-89 /
//! GOST R 34.12-2015 "Magma" 64-bit block cipher (ECB/CBC, 256-bit key, hex/base64
//! I/O). Thin wrapper; chat schema single-sourced from descriptor(); handler
//! delegates to run_skill. Pure → all backends.
#![cfg_attr(not(target_arch = "wasm32"), allow(dead_code, unused_imports))]
use gizza_ai_gost_magma_cipher_core::{decrypt, encrypt, Encoding, Mode};
...
fn descriptor() -> ToolDescriptor {
    ToolDescriptor::new(Input::None)
        ...
        .param(
            Param::enumv("cipher", ["cbc", "ecb"]).default("cbc").describe(
                "Mode of operation: cbc (default) or ecb (insecure — reveals patterns; avoid).",
            ),
        )
        .param(Param::string("key").required().describe(
            "The 32-byte (256-bit) Magma key, encoded with `format`.",
        ))
        .param(Param::string("iv").describe(
            "The 8-byte IV, encoded with `format`. Required for cbc; omit for ecb.",
        ))
        .param(
...
impl GostMagmaCipher {
    fn handle(_msg: Message, body: Vec<u8>) -> GuestResult {
        match run_skill(&body, "gost-magma-cipher", |a: Args| {
        ...
        let authored: serde_json::Value = serde_json::from_str(
            r#"{
                ...
                "properties": {
                    ...
                    "cipher": { "type": "string", "enum": ["cbc", "ecb"], "default": "cbc", "description": "Mode of operation: cbc (default) or ecb (insecure — reveals patterns; avoid)." },
                    "key": { "type": "string", "description": "The 32-byte (256-bit) Magma key, encoded with `format`." },
