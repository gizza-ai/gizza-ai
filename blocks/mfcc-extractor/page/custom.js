...
// field inputs, so this small adapter reads the uploaded audio file as base64
// and then calls the wasm-bindgen export with the same argument order as
// web/src/lib.rs.
let selectedBase64 = "";
let runSeq = 0;
  ...
  function gatherArgs() {
    ...
    for (const input of cfg.inputs) {
      if (input.source === "file") {
        args.push(selectedBase64);
      } else {
        args.push(helpers.readField(document.getElementById(input.elementId)));
      }
    }
    return args;
  }
