//! Browser-facing wasm-bindgen wrapper for /tools/mp4-to-m4a/ (ffmpeg page).
...
pub fn build_argv(in_name: &str) -> Result<JsValue, JsValue> {
    let (argv, out_name) = gizza_ai_mp4_to_m4a_core::plan(in_name).map_err(|e| JsValue::from_str(&e))?;
