//! gizza-ai/video-title-card — overlay a styled title / lower-third caption onto
//! a video over a chosen time range, via ffmpeg `drawtext`.
//!
//! The chat schema is derived from `descriptor()` (single source — shared shape
//! across chat + CLI + page); the handler delegates source-resolution, ffmpeg
//! dispatch, and envelope-building to `block_utils`. Text/position/color/timing
//! validation and the pure argv builder live in `core`, shared with the page.
//!
//! The overlay text is written to ffmpeg's virtual FS as a `textfile` and the
//! bundled font as a `fontfile` (both via [`dispatch_ffmpeg_inputs`]) — so the
//! filtergraph carries no user text (no escaping / injection) and the browser
//! ffmpeg, which has no system fonts, still renders. NOTE: chat ffmpeg is
//! non-functional (the chat runtime is a Service Worker where ffmpeg can't
...
fn descriptor() -> ToolDescriptor {
    ToolDescriptor::new(Input::Video)
        .param(
            Param::string("text")
                .required()
                .describe("The title or lower-third caption to overlay. Drawn literally, so apostrophes, colons and % are fine — no escaping needed. Keep it to one short line."),
        )
        .param(
            Param::enumv(
                "position",
            ...
            .describe("Where to anchor the text: top-left, top-center, top-right, center, bottom-left, bottom-center or bottom-right. Default bottom-center (classic lower-third)."),
        )
        .param(
            Param::integer("font_size")
                ...
                .max(400.0)
                .default(48)
                .describe("Font size in pixels (8-400). Default 48."),
        )
        .param(
            Param::string("font_color")
                .default("#ffffff")
                .describe("Text color: a CSS color name (white, black, navy, ...) or hex like #FFCC00 / #fc0. Default #ffffff (white)."),
        )
        .param(
            Param::boolean("background")
                .default(true)
                .describe("Draw a semi-transparent box behind the text (the lower-third bar look). Default true."),
        )
        .param(
        ...
        .param(
            Param::number("start")
                .min(0.0)
                .default(0.0)
                .describe("Seconds into the video when the caption appears. Default 0 (from the start)."),
        )
        .param(
            Param::number("end")
                .min(0.0)
                .default(5.0)
                .describe("Seconds into the video when the caption disappears. Default 5. Must be greater than start."),
        )
}
...
fn run(body: Vec<u8>) -> Result<Vec<u8>, SkillError> {
    let args: Args = serde_json::from_slice(&body).invalid_args("video-title-card")?;
    let position = args.position.as_deref().unwrap_or("bottom-center");
    let font_size = args.font_size.unwrap_or(48);
    ...
    let end = args.end.unwrap_or(5.0);
    ...
    // Write the media, the bundled font, and the overlay text into ffmpeg's FS.
    let inputs = vec![
    ...
    let filename = filename_with_suffix(&in_filename, "-title", out_ext);
    let for_llm = format!(
        "added title \"{}\" ({position}, {start}-{end}s) to {in_filename} ({output_size} bytes {out_mime})",
        args.text
    );
...
mod tests {
    ...
    /// schema (Input::Video url⊕ref oneOf + the nine styling params), so any
        ...
        let authored: serde_json::Value = serde_json::from_str(
            r##"{
                ...
                "properties": {
                    ...
                    "text":   { "type": "string", "description": "The title or lower-third caption to overlay. Drawn literally, so apostrophes, colons and % are fine — no escaping needed. Keep it to one short line." },
                    "position": {
                        "type": "string",
                    ...
                    "font_size": { "type": "integer", "minimum": 8, "maximum": 400, "default": 48, "description": "Font size in pixels (8-400). Default 48." },
                    "font_color": { "type": "string", "default": "#ffffff", "description": "Text color: a CSS color name (white, black, navy, ...) or hex like #FFCC00 / #fc0. Default #ffffff (white)." },
                    "background": { "type": "boolean", "default": true, "description": "Draw a semi-transparent box behind the text (the lower-third bar look). Default true." },
                    ...
                    "start": { "type": "number", "minimum": 0, "default": 0.0, "description": "Seconds into the video when the caption appears. Default 0 (from the start)." },
                    "end":   { "type": "number", "minimum": 0, "default": 5.0, "description": "Seconds into the video when the caption disappears. Default 5. Must be greater than start." }
                },
                "required": ["text"],
    ...
    #[test]
    fn output_filename_uses_title_suffix() {
        assert_eq!(filename_with_suffix("clip.mp4", "-title", "mp4"), "clip-title.mp4");
    }
}
