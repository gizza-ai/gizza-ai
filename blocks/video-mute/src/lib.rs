//! gizza-ai/video-mute — remove the audio track from a video via ffmpeg.
//! Lossless (stream-copies the video, drops audio). Source-resolution, ffmpeg
//! dispatch, and envelope-building delegated to block_utils. NOTE: chat ffmpeg
//! is non-functional (Service Worker) — page + CLI surfaces.
...
use gizza_ai_video_mute_core::build_argv;
...
fn run(body: Vec<u8>) -> Result<Vec<u8>, SkillError> {
    let args: Args = serde_json::from_slice(&body).invalid_args("video-mute")?;
    let (input_bytes, in_mime, in_filename) =
        resolve_source(args.source.into_inner(), AssetKind::Video, MAX_INPUT_BYTES)?;
    let in_ext = mime_to_ext(&in_mime).unwrap_or("mp4");
    let ffmpeg_in = format!("in.{in_ext}");
    let (argv, ffmpeg_out) = build_argv(&ffmpeg_in);
    ...
    let for_llm = format!("muted {in_filename} ({output_size} bytes {out_mime})");
