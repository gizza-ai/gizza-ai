//! gizza-ai/mp4-to-mov — fetch an MP4 URL or attachment ref, rewrap it into a
//! QuickTime (.mov) container via ffmpeg (lossless stream-copy), and return the
//! media envelope.
//!
//! `-map 0 -c copy -movflags +faststart` copies every stream (video + all audio
//! + data) into the QuickTime container with no re-encode: lossless,
//! near-instant, no quality change. `.mov` is the ISO-BMFF ancestor of MP4 and a
//! superset container that accepts every codec MP4 can hold, so the remux always
//! succeeds — there is no transcode fallback (and no params). The point of `.mov`
//! is Apple-native workflows: Final Cut Pro, iMovie and QuickTime Player expect
//! it. To actually re-encode (change codec/quality), use video-transcode.
//!
...
use gizza_ai_mp4_to_mov_core::build_argv;
...
#[wafer_block(
    ...
    summary = "Remux an MP4 video into a QuickTime (.mov) container losslessly.",
...
fn run(body: Vec<u8>) -> Result<Vec<u8>, SkillError> {
    ...
    let args: Args = serde_json::from_slice(&body).invalid_args("mp4-to-mov")?;
    let (out_mime, out_ext) = format_to_mime_and_ext(AssetKind::Video, "mov")
        .expect("video/quicktime is a known format");

    // 2. Resolve source — URL fetch or attachment lookup.
    let (input_bytes, in_mime, in_filename) =
        resolve_source(args.source.into_inner(), AssetKind::Video, MAX_INPUT_BYTES)?;

    // 3. Build ffmpeg argv (shared pure core). Output is always out.mov.
    ...
    let for_llm = format!(
        "remuxed {in_filename} ({in_mime}) into a QuickTime container {out_mime} ({output_size} bytes, lossless -c copy)"
    );
...
mod tests {
    ...
    fn output_filename_swaps_extension_to_mov() {
        ...
        assert_eq!(replace_extension("MyMovie.M4V", "mov"), "MyMovie.mov");
    }
}
