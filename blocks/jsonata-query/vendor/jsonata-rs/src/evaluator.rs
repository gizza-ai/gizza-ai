...
impl<'a> Evaluator<'a> {
    ...
    fn check_limits(&self, inc_or_dec: bool) -> Result<()> {
        ...
        if let Some(started_at) = internal.started_at {
            ...
            // implemented on this platform"), which traps as `unreachable` and
            // breaks the in-browser page. On wasm we simply skip the elapsed-time
            // guard (the depth limit below still bounds runaway evaluation).
