//! gizza-ai/mov-to-mp4 — fetch a MOV (QuickTime) URL or attachment ref, rewrap
//! it into an MP4 container via ffmpeg, and return the media envelope.
//!
//! Default is a lossless `-c copy` stream-copy remux (fast, no quality change —
//! works for the H.264/HEVC + AAC codecs essentially every iPhone/camera MOV
//! uses); `mode = "transcode"` re-encodes to H.264/AAC as an explicit fallback
//! for MOVs carrying codecs MP4 can't hold (e.g. Apple ProRes). Unlike
//! video-transcode / video-compress, which ALWAYS re-encode, the headline here
//! is the stream-copy remux.
//!
...
use gizza_ai_mov_to_mp4_core::{build_argv, parse_mode, quality_to_crf, DEFAULT_QUALITY};
            ...
            Param::enumv("mode", ["copy", "transcode"])
                ...
                .describe(
                    "How to produce the MP4. copy (default) = lossless container remux with \
                     -c copy: near-instant, no quality change, works when the MOV already holds \
                     H.264/HEVC video + AAC audio (nearly all iPhone/camera clips). transcode = \
                     re-encode to H.264/AAC — use for MOVs whose codecs MP4 can't hold, e.g. \
                     Apple ProRes.",
                ),
        )
        .param(
...
fn run(body: Vec<u8>) -> Result<Vec<u8>, SkillError> {
    ...
    let args: Args = serde_json::from_slice(&body).invalid_args("mov-to-mp4")?;
    let mode_str = args.mode.as_deref().unwrap_or("copy");
    let mode = parse_mode(mode_str)
        .map_err(|e| SkillError::InvalidArgs(format!("invalid mov-to-mp4 args: {e}")))?;
    validate_quality_1_100(args.quality, "mov-to-mp4")?;
    let crf = quality_to_crf(args.quality.unwrap_or(DEFAULT_QUALITY));
    let (out_mime, out_ext) = format_to_mime_and_ext(AssetKind::Video, "mp4")
        .expect("video/mp4 is a known format");
    ...
    let in_ext = mime_to_ext(&in_mime).unwrap_or("mov");
    ...
    let for_llm =
        format!("rewrapped {in_filename} ({in_mime}) to {out_mime} via {mode_str} ({output_size} bytes)");
...
mod tests {
        ...
        let authored: serde_json::Value = serde_json::from_str(
            r#"{
                ...
                "properties": {
                    ...
                    "mode":    { "type": "string", "enum": ["copy", "transcode"], "default": "copy", "description": "How to produce the MP4. copy (default) = lossless container remux with -c copy: near-instant, no quality change, works when the MOV already holds H.264/HEVC video + AAC audio (nearly all iPhone/camera clips). transcode = re-encode to H.264/AAC — use for MOVs whose codecs MP4 can't hold, e.g. Apple ProRes." },
    ...
    fn output_filename_swaps_extension_to_mp4() {
        ...
        assert_eq!(replace_extension("MyMovie.MOV", "mp4"), "MyMovie.mp4");
    }
}
