//! gizza-ai/blowfish-cipher — encrypt or decrypt data with Blowfish (ECB or CBC).
...
use gizza_ai_blowfish_cipher_core::{decrypt, encrypt, Encoding, Mode};
...
fn descriptor() -> ToolDescriptor {
    ToolDescriptor::new(Input::None)
        ...
        .param(
            Param::enumv("cipher", ["cbc", "ecb"]).default("cbc").describe("Blowfish mode of operation: cbc (default) or ecb."),
        )
        .param(Param::string("key").required().describe("The Blowfish key (4–56 bytes), encoded with `format`."))
...
#[wafer_block(
    ...
    summary = "Encrypt or decrypt data with Blowfish (ECB/CBC)",
    skill(
...
impl BlowfishCipher {
    fn handle(_msg: Message, body: Vec<u8>) -> GuestResult {
        match run_skill(&body, "blowfish-cipher", |a: Args| {
        ...
        let authored: serde_json::Value = serde_json::from_str(
            r#"{
                ...
                "properties": {
                    ...
                    "cipher": { "type": "string", "enum": ["cbc", "ecb"], "default": "cbc", "description": "Blowfish mode of operation: cbc (default) or ecb." },
                    "key": { "type": "string", "description": "The Blowfish key (4–56 bytes), encoded with `format`." },
